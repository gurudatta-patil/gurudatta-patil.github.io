...
function getApiBase() {
  const configured = window.CAGR_CONFIG?.apiBase?.trim() || '';
...
export async function fetchCurrentPrices(symbols) {
  ...
  if (toFetch.length) {
    ...
    const data = await fetch(apiUrl(`/api/price/current?symbols=${qs}`)).then(r => r.json());
...
export async function fetchHistoricalPrices(symbol, startDate, endDate) {
  ...
  const data = await fetch(
    apiUrl(`/api/price/history?symbol=${encodeURIComponent(symbol)}&start=${startDate}&end=${endDate}`)
  ).then(r => r.json());
...
export async function validateTicker(ticker) {
  return fetch(apiUrl('/api/price/validate'), {
    method:  'POST',
